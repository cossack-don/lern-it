...
<!-- Добавить flex-wrapp и сделать адаптив под приложение -->
<template>
  <div id="app">
    
<!--   error   -->
    <div v-if="error" class="main-error"> {{error}}</div>
    <div v-if="errorSearch" class="error-search" >{{errorSearch}}</div>
    
<!--   wrapper- для кнопок и поиска   -->
    <div class="wrapper-content ">
      
      <div v-if="nickGithub" >Вы ввели ник <span style="color:#ffffff; margin-left:5px; margin-right:5px; font-weight: bold;">{{nickGithub}}</span> пользователя гитхаба</div>
      <span v-else class="main-title">Введите ник GitHub</span>
    
<!--    Кнопки и поиск    -->
      <input v-model.trim="valueSearch" @keyup.enter="searchRepos"  placeholder="Введите ник GitHub" class="main-input mt-15" >
      <button @click="searchRepos" class="mt-15" >Поиск</button>
      <button @click="cleanSerchAndListRepos" class="mt-15 btn-clean-all" :disabled="disabled = !repos">Очистить все</button>

    </div>
<!--  Оповищение сколько репозиториев шт    -->
    <div v-if="repos && repos.length !== 0" class="count-repos">У пользователя <span style="color:#2c3e50;">{{nickGithub}}</span> -
...
<!--       svg картинка - звезда     -->
            <div class="star">
              <svg  id="Capa_1"  xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
...
<!--    ссылка на репозиторий        -->
           <a target="_blank" :href="item.html_url" class="link-repos">{{item.name}}</a>
<!--     дата создания репозитория       -->
          <div class="create-repos">Создан: {{cleanDataAndReturnNewDataCreateProdject(item.created_at)}}</div>
         </div>
    </div>
...
//     метод для очищения полученной даты, в человеческий вид
    cleanDataAndReturnNewDataCreateProdject(str) {
        ...
        const newStringData = cleanDataString.substr(8, 10)
        + '-' + cleanDataString.substr(5, 2) + '-' + cleanDataString.substr(0, 4);
...
#app {
  ...
  .wrapper-content {
    padding:35px;
    background: rgba(106, 224, 171, 1.0);
    background: -webkit-linear-gradient(top left, rgba(106, 224, 171, 1.0), rgba(11, 34, 78, 1.0));
    background: -moz-linear-gradient(top left, rgba(106, 224, 171, 1.0), rgba(11, 34, 78, 1.0));
    background: linear-gradient(to bottom right, rgba(106, 224, 171, 1.0), rgba(11, 34, 78, 1.0));
    margin:0 auto;
    max-width:250px;
    display:flex;
  ...
  .main-item-repos {
    background: rgba(150, 232, 195, 1.0);
    background: -webkit-linear-gradient(top left, rgba(150, 232, 195, 1.0), rgba(166, 191, 242, 1.0));
    background: -moz-linear-gradient(top left, rgba(150, 232, 195, 1.0), rgba(166, 191, 242, 1.0));
    background: linear-gradient(to bottom right, rgba(150, 232, 195, 1.0), rgba(166, 191, 242, 1.0));
    /* margin:10px; */
    padding:15px;
    border-radius: 1em;
    display:flex;
